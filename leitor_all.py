...
class leitor_pdf_inter:
    ...
    def leitor_pdf_inter_v2(self, pdf):
        ...
        with open('banco_inter_csv', 'r', encoding='utf-8') as file:
            reader = csv.reader(file)
            for row in reader:
                lista.append(row)
...
class leitor_pdf_SICOOB:
    ...
    def lendo_pdf_sicoob_CODIGO_IA(self, pdf_path: str):
        """
        Lê todas as páginas do PDF `pdf_path` e retorna lista de objetos Linha,
        com campos: instituição, data, descrição (completa), valor, (vazios), mês, ano, primeiro_dia_mes.
        """
        # 1. Abre PDF para contar páginas
        try:
        ...
        # 2. Regex para extrair data, descrição multiline e valor
        pattern = re.compile(
            r"(\d{2}/\d{2})\s+([\s\S]+?)(?=\s+[\d\.]+,\d{2}[DC])\s+([\d\.]+,\d{2})([DC])",
            re.MULTILINE
        )
        ...
        # 3. Itera sobre todas as páginas
        for page_idx in range(num_pages):
            with pdfplumber.open(pdf_path) as pdf:
                text = pdf.pages[page_idx].extract_text() or ''

            # 4. Captura cada movimento
            for m in pattern.finditer(text):

                print(pattern)

                dia_mes   = m.group(1)           # "DD/MM"
                raw_desc  = m.group(2).strip()   # pode conter quebras de linha
                # Normaliza descrição (remover múltiplos espaços e novas linhas)
                descricao = re.sub(r"\s+", " ", raw_desc)
                raw_valor = m.group(3)           # ex: "1.234,56"
                sinal     = m.group(4)           # 'D' ou 'C'

                # 5. Converte para float e aplica sinal
                valor = float(raw_valor.replace('.', '').replace(',', '.'))
                if sinal == 'D':
                    valor = -valor

                # 6. Determina ano e primeiro dia do mês
                mes = dia_mes[3:5]
                mes_int = int(mes)
                ano = agora.year if mes_int <= agora.month else agora.year - 1
                data_completa = f"{dia_mes}/{ano}"      # ex: "01/11/2024"
                primeiro_dia  = f"01/{mes}/{ano}"       # ex: "01/11/2024"

                # 7. Monta objeto Linha
                row = Linha(
                    'Sicoob',            # instituição
                    data_completa,
                    descricao,
        ...
        # 8. Retorna lista de objetos Linha
        return resultados

    def lendo_pdf_sicoob_IA(self, pdf_path: str):

        resultados = []
        agora = datetime.now()

        # Regex para identificar início de transação (ex: 18/06)
        data_regex = re.compile(r"^\d{2}/\d{2}")
        # Regex para identificar valor e D/C no final (ex: R$ 1.234,56D)
        valor_regex = re.compile(r"R\$ ([\d\.]+,\d{2})([DC])$")
            ...
            for page in pdf.pages:
                ...
                while i < len(linhas):
                    ...
                    if data_regex.match(linha):
                        dia_mes = linha[:5]
                        descricao_linhas = [linha[6:].strip()]  # parte da descrição na mesma linha da data
                        i += 1

                        # Continua juntando as linhas seguintes até encontrar o valor
                        while i < len(linhas):
                            l = linhas[i].strip()

                            if valor_regex.search(l):
                                valor_match = valor_regex.search(l)
                                raw_valor = valor_match.group(1)
